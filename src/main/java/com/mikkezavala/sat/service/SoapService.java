...
import static com.mikkezavala.sat.util.XmlUtil.WSS_MESSAGE_SECURITY_NS;
import static com.mikkezavala.sat.util.XmlUtil.WSS_SEC_EXT_NS;
import static com.mikkezavala.sat.util.XmlUtil.WSS_TOKEN_PROFILE_NS;
import static com.mikkezavala.sat.util.XmlUtil.WSS_UTILITY_NS;
...
public class SoapService {
    ...
    public SOAPMessage solicita(String rfc) throws Exception {
        ...
        request.setFechaInicial(startDate);
        request.setRfcReceptor(rfc);
        request.setRfcSolicitante(rfc);
        requestDownload.setRequest(request);

        SOAPBody body = message.getSOAPBody();
        SOAPElement descargaNode = soapHandler.createNodeOf(requestDownload);

        createDetachedSignature(getFirstNode(descargaNode), null, rfc);
        body.addChildElement(descargaNode);
    ...
    public SOAPMessage valida(String idSolicitud, String rfc) throws Exception {
        Request request = new Request();
        SOAPMessage message = soapHandler.createEnvelope();
        ValidateDownload verificaSolicitud = new ValidateDownload();

        request.setId(idSolicitud);
        request.setRfcSolicitante(rfc);
        verificaSolicitud.setRequest(request);

        SOAPBody body = message.getSOAPBody();
        SOAPElement verificaSolicitudNode = soapHandler.createNodeOf(verificaSolicitud);
    ...
    public SOAPMessage descarga(String idSolicitud, String rfc) throws Exception {
        DownloadRequest peticion = new DownloadRequest();
        SOAPMessage message = soapHandler.createEnvelope();
        Download peticionMasiva = new Download();

        peticion.setIdPaquete(idSolicitud);
        peticion.setRfcSolicitante(rfc);
        peticionMasiva.setDownloadRequest(peticion);
